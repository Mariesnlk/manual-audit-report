...
contract UnidoDistribution is Ownable {
    ...
    mapping(POOL => uint256) public pools;
    ...
    function addParticipants(
        ...
        require(
            pool >= POOL.SEED && pool <= POOL.RESERVE,
            "Must select a valid pool"
        );
        require(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(
                lockoutBalances[p] == 0,
                "Participants can't be involved in multiple lock ups simultaneously"
            );
    ...
    function updateRelease() external onlyOwner returns (bool) {
        ...
        for (uint256 i = continuePoint; i < len && i < continueAddScan; i++) {
            ...
            // TODO finished lockout period (lockoutPeriods) depends on how much time the owner of the contract calls it
            if (lockoutPeriods[p] > 0) {
                lockoutPeriods[p]--;
            } else if (lockoutReleaseRates[p] > 0) {
                uint256 rate = lockoutReleaseRates[p];
                ...
                // TODO finished lockoutReleaseRates depends on how much time the owner of the contract calls it
        ...
        // TODO continuePoint always increase - maybe overflow
        // TODO add some require for continuePoint
